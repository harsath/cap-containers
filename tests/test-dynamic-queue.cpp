...
TEST(Cap_Container, Cap_Container_dynamic_queue){
	{
		...
		ASSERT_EQ(cap_dynamic_queue_get_size(dynamic_queue), 0);
		int value_one = 10;
		int value_two = 20;
		int value_three = 30;
		// [30] [20] [10]
		...
		ASSERT_EQ(cap_dynamic_queue_get_size(dynamic_queue), 3);
		...
		// [30]
		ASSERT_EQ(cap_dynamic_queue_get_size(dynamic_queue), 1);
