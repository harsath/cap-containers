...
void test_list(void){
	{
		...
		CAP_ASSERT_EQ(*((int*)cap_list_front(d_list_one)), value_one, "LIST front");
		CAP_ASSERT_EQ(*((int*)cap_list_back(d_list_one)), value_one, "LIST back");

		int value_two = 20; // [head] [20] [10] [tail]
		cap_list_push_front(d_list_one, &value_two);
		CAP_ASSERT_EQ(*((int*)cap_list_front(d_list_one)), value_two, "LIST front");
		CAP_ASSERT_EQ(*((int*)cap_list_back(d_list_one)), value_one, "LIST back");

		int value_three = 30; // [head] [30] [20] [10] [tail]
		cap_list_push_front(d_list_one, &value_three);
		CAP_ASSERT_EQ(*((int*)cap_list_front(d_list_one)), value_three, "LIST front");
		CAP_ASSERT_EQ(*((int*)cap_list_back(d_list_one)), value_one, "LIST");

		CAP_ASSERT_EQ(*((int*)cap_list_pop_back(d_list_one)), value_one, "LIST pop_back"); // [head] [30] [20] [tail]
		CAP_ASSERT_EQ(*((int*)cap_list_back(d_list_one)), value_two, "LIST back");
		...
		CAP_ASSERT_TRUE((find_node_value_two != NULL) && (*(int*)find_node_value_two == value_two), "LIST find_if");
		CAP_ASSERT_TRUE(cap_list_remove_if(d_list_one, predicate_one), "LIST remove_if"); // [head] [30] [40] [tail]
		CAP_ASSERT_TRUE((*(int*)cap_list_back(d_list_one) == value_three) == (*(int*)cap_list_front(d_list_one) == value_four),
					"LIST back");
...

	{
		...
		CAP_ASSERT_TRUE(find_if_predicate_two != NULL, "LIST find_if null check");
		bool find_if_predicate_two_is_address_correct = 
			strcmp(one_user_address, find_if_predicate_two->user_addres) == 0 ? true : false;
		CAP_ASSERT_TRUE(find_if_predicate_two_is_address_correct, "LIST find_if");
		cap_list_remove_if(d_list_two, predicate_two);
		CAP_ASSERT_TRUE(cap_list_find_if(d_list_two, predicate_two) == NULL, "LIST find_if");
